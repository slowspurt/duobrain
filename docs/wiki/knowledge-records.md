...
Importance evidence is locally verified when `evidenceRef` is a supplied `wiki/<uuid>.md`, `wiki:<uuid>`, `wiki:wiki/<uuid>.md`, `ticket:<id>`, or an exact supplied ticket ID. A recognized wiki/ticket ref that is absent becomes `state: "unverified"`; an arbitrary external ref becomes `state: "caller-claim"`. Both retain the requested score for inspection but do not apply it to exposure, use the unknown-importance policy default, and emit a warning. This distinguishes a caller assertion from locally verified evidence without requiring a new note format.
...
No record is omitted. Lower exposure only changes its index section. Source paths, author/status metadata, unresolved-ticket links, and agreement evidence remain visible, and the original Markdown strings are never changed. Before returning a writable candidate, W4 traces every included wiki source, `summarizes`, `decisionEvidence`, `previousSummary`, and `supersedes` edge. Missing targets, self-reference, and cycles remain in `issues` as errors and make the outcome `insufficient`; they are not silently presented as a verified refinement. Importance-evidence warnings alone do not block a plan because their score is not applied.
...
The run key excludes candidate UUID and observation time. Repeating the same date, timezone, source revision, and resolved policy returns `outcome: "duplicate"` only when the referenced summary actually exists in `notes` and its `dailyRefinement` date, timezone, source revision, policy fingerprint, and run key all match. A run key alone is not completion evidence. If `priorRun` claims the current run but its summary is absent or mismatched, W4 emits `PRIOR_RUN_SUMMARY_NOT_PERSISTED` and prepares a recovery candidate with no false prior-summary lineage. A changed source revision or policy creates a new plan. These checks add fields and issues without changing the existing engine-facing input or result names. See `examples/wiki/daily-refinement-policy.json` for the caller-owned policy and prior-run shape.
