...
| Wiki refinement | On-demand, evidence-based importance and recency; preserved sources and decisions; immutable summary indexes; either participant may run it; unchanged date/source/policy/timezone is skipped; delivery-aware retries. |
...
- **G4:** a missing-method request is proposed, created, reused, answered and compared again. Manual refinement pushes a summary, skips an unchanged same-day rerun and links the next day's summary. The peer sees records after sync. A successful comparison does not itself resolve the ticket.
...
AI execution uses the user's existing AI tool and the supplied guidance. There is no installed background AI service or automatic wake-up. Wiki refinement runs only when a participant invokes `wiki-refine`.
