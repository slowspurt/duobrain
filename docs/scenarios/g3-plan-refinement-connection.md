...
| “내 범위 바꿨어.” | B records `session.scope_updated`; the projected scope changes while the original scope and reason remain in session history. | Product code and branch are untouched. D3 currently treats a history containing the scope-update event as unknown time, so per-scope time attribution is not complete. |
...
The same run calls D3 aggregation to preserve the current gap rather than hide it: the scope-updated session remains time `unknown`, and the example reports `scopeUpdateAttributionComplete: false`. W4 is also only a pure planner. The call leaves the shared-store HEAD and every source Markdown byte unchanged, and its candidate is not present in Git afterward.
