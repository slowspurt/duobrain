...
화면의 탭, 버튼, 상태명, 안내 문구와 날짜·시간 형식은 브라우저 언어를 따른다. 현재 한국어와 영어를 지원하며, 지원하지 않는 언어는 영어로 표시한다. 재현 가능한 데모와 검토에는 주소 뒤에 `?lang=ko` 또는 `?lang=en`을 붙여 언어를 고정할 수 있다. 티켓 본문, 목표, 병목, 위키 원문처럼 저장소에 기록된 내용은 번역하지 않는다.
