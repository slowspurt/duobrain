...
화면은 보이는 동안 30초마다 `/api/snapshot`을 다시 읽고, 내용이 바뀐 경우에만 다시 그려 열어 둔 상세와 근거가 유지된다. 상단의 `Updated …` 버튼으로 즉시 새로 읽을 수 있으며, 서버 연결이 끊기면 마지막으로 읽은 기록을 유지한 채 오류 안내를 표시한다. `#current`, `#requests`, `#records`, `#wiki` 주소는 해당 탭을 바로 연다.

`Needs attention`의 `Viewing as`에서 참여자를 고르면(브라우저에 저장) 그 사람이 움직여야 할 요청을 `Your turn`으로 표시하고 맨 위로 올린다. 차례는 티켓 상태만으로 정한다: `open`·`acknowledged`는 받는 사람이 답할 차례, `needs_information`·`answered`는 요청한 사람이 보충하거나 해결을 확인할 차례이며, 종료된 요청은 누구의 차례도 아니다. 선택하지 않으면 각 요청에 `Waiting on <사람>`만 표시한다.
