...
The engine provides explicit overlap assessment and product-worktree preparation, live wiki search/comparison, and an on-demand wiki refinement command with timezone, duplicate and retry checks ([engine guide](engine/README.md#wiki-refinement-execution)). The dashboard summarizes recorded intervals and declared blockers, not measured productivity. No automatic transcript capture or background AI service is installed. The isolated shared-store checkout remains separate from product code worktrees.
