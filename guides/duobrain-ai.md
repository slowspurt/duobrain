...
첫 설정, 진행 중 프로젝트 판별과 중단 후 재개는 [AI 온보딩 시작 지침](duobrain-onboarding.md)에서 시작한다.
...
1. `onboarding-inspect`로 재개 단계를 읽고, 기존 기획, 결정 기록, 단계별 계획을 읽어 새 프로젝트인지 진행 중 프로젝트인지 근거와 함께 제안한다. 커밋 수만으로 진행 중이라고 확정하지 않는다. 목표·현재 단계·두 사람의 담당 범위·미정 사항을 요약하고 기존 합의나 개인 선택을 AI가 새 합의로 바꾸지 않는다.
