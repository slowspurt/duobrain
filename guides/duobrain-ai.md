...
Run commands from the duobrain checkout: `node bin/duobrain.js <command> --repository <product-path>`. Commands print JSON; `<command> --help` shows every option. If duobrain is not set up for this clone, follow [the onboarding guide](duobrain-onboarding.md) first.
